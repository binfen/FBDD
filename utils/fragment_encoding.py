...
import numpy as np
from .fragment_tree import build_tree_from_list
import bisect
from .fragment_utils import split_molecule, join_fragments
...
def save_decodings(decodings):
    ...
    if not os.path.exists("../utils/data/frag_lib/"):
        os.makedirs("../utils/data/frag_lib")

    with open("../utils/data/frag_lib/decodings.txt","w+") as f:
        f.write(str(decodings_smi))

# Read encoding list from file
def read_decodings():
    with open("../utils/data/frag_lib/decodings.txt","r") as f:
        d = eval(f.read())
