    ...
    if setting == 'imputation':
        ...
        for fold_x in range(int(n_fold ** 0.5)):
            for fold_y in range(int(n_fold ** 0.5)):
                ...
                test_idx_list.append(test_idx)
                print
                'fold', fold_x * int(n_fold ** 0.5) + fold_y, 'train ', len(train_idx), 'test ', len(
                    test_idx), 'valid ', len(valid_idx)
...
def loading_emb(measure):
    ...
    print
    'atom dict size:', len(atom_dict), ', bond dict size:', len(bond_dict), ', word dict size:', len(word_dict)
